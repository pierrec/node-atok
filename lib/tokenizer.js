 ...
 * - options (Object):
 * - options._bufferMode (Boolean): use Buffers instead of string (false)
 * - options._bufferData (Boolean): buffer data if handler holds up the processing (false)
 *
**/
function Tknzr (options) {
  ...
  this._bufferData = (options.bufferData === true)
 ...
 * Tokenizer#flush()
 *
 * Terminate the current tokenizing and return the flushed buffer
**/
...
Tknzr.prototype.write = function (data) {
  ...
  if (this._bufferMode)
    ...
    this.buffer += data.toString( this._encoding )
  ...
  return this.paused ? this._bufferData : this._tokenize()
}
...
Tknzr.prototype._tokenize = function () {
  ...
  if (this.offset < this.buffer.length) {
    for (
        var i = 0, p, matched
      ; this.offset < this.buffer.length && i < this.rules.length
      ; i++
      )
    {
      p = this.rules[i]
      ...
      if ( matched >= 0 ) {
        ...
        if (this.paused) return this._bufferData
 ...
 * - firstMatch (String | Integer | Array): rule (String: expect string, Integer: expect n characters, Array: expect one of the string items)
 * - nextMatch (String | Integer | Array): rule (String: expect string, Integer: expect n characters, Array: expect one of the string items)
 * - type (String | Function): rule name (referenced in default handler execution) or handler (executed on full match)
 *
 * TODO
**/
// {String} start match {String} type
// {String} start match {String} end match {String} type
// {Integer} expected num bytes {String} type
// {Integer} expected num bytes {String} end match {String} type
Tknzr.prototype.addRule = function (/*rule1, rule2, ... type|handler*/) {
  ...
  if ( args[0] === 0 ) {
    ...
    this.rules.push(
      // new (this._bufferMode ? RuleBuffer : RuleString)(
      RuleString(
        args
      , type
      , handler
 ...
 * Triggered if the handler was not supplied on #addRule, and not #ignore'd
 * TODO - should be have as property
**/
Tknzr.prototype.setDefaultHandler = function (handler) {
  // if ( typeof handler !== 'function' )
  //   return this._error( new Error('Tokenizer#setDefaultHandler: Handler not a function') )
